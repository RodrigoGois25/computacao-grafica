// Arquivo: js/app.js (VERSÃO FINAL COM TRANSFORMAÇÕES EM TEMPO REAL)
...
async function handleCanvasClick(event) {
    ...
    switch (drawingMode) {
        case 'bresenham': case 'circulo': case 'curva':
            tempPoints.push({ x, y }); await processShapeDrawing(x, y); break;
        case 'poligono':
            if (event.detail === 2) { if (tempPoints.length > 2) { saveState(); await finalizePolygon(); } } else { tempPoints.push({ x, y }); drawTempPolygon(); } break;
        case 'preenchimento':
            await floodFill(x, y, corPreenchimento, grid); renderGrid(); break;
        case 'scanline':
            if (polygons.length > 0) { await scanlineFill(polygons[polygons.length - 1], corPreenchimento, grid); renderGrid(); } break;
        case 'recorte':
...
async function processShapeDrawing(x, y) {
    renderGrid();
    if ((drawingMode === 'curva' && tempPoints.length < 4) || tempPoints.length < 2) {
        tempPoints.forEach(p => { ctx.fillStyle = corControle; ctx.fillRect(p.x * PIXEL_SIZE, p.y * PIXEL_SIZE, PIXEL_SIZE, PIXEL_SIZE); });
        return;
    }
    if (drawingMode === 'bresenham') { await bresenham(tempPoints[0].x, tempPoints[0].y, x, y); }
    else if (drawingMode === 'circulo') {
        const dx = x - tempPoints[0].x; const dy = y - tempPoints[0].y;
        ...
        for (let i = 0; i < bezierPoints.length - 1; i++) { await bresenham(bezierPoints[i].x, bezierPoints[i].y, bezierPoints[i + 1].x, bezierPoints[i + 1].y); }
    }
    tempPoints = [];
    renderGrid();
}
...
async function finalizePolygon() {
    ...
    for (let i = 0; i < tempPoints.length; i++) {
        const p1 = tempPoints[i]; const p2 = tempPoints[(i + 1) % tempPoints.length];
...
async function handleClippingClick(x, y) {
    if (clippingState === 'defining_window') {
        ...
        tempPoints.push({ x, y });
        if (tempPoints.length === 1) { ctx.fillStyle = corControle; ctx.fillRect(x * PIXEL_SIZE, y * PIXEL_SIZE, PIXEL_SIZE, PIXEL_SIZE); }
...
document.querySelectorAll('input[name="tool"]').forEach(radio => { radio.addEventListener('change', e => { drawingMode = e.target.value; isCubeActive = false; tempPoints = []; if (drawingMode === 'recorte') { clippingState = 'defining_window'; clipWindow = {}; } renderGrid(); if (drawingMode === 'recorte') drawClipWindow(); }); });
...
function redrawAllPolygons() {
    ...
    polygons.forEach(poly => {
        for (let i = 0; i < poly.length; i++) {
            ...
            const lineSegment = bresenhamSync(p1.x, p1.y, p2.x, p2.y);
            lineSegment.forEach(p => drawToGrid(p, corDesenho));
        }
    });
    renderGrid();
}

// --- LÓGICA DE TRANSFORMAÇÕES E CUBO (ATUALIZADA) ---
const drawCubeBtn = document.getElementById('drawCubeBtn');
const txInput = document.getElementById('tx'); const tyInput = document.getElementById('ty');
const sxInput = document.getElementById('sx'); const syInput = document.getElementById('sy');
const angleInput = document.getElementById('angle');

// Função unificada para aplicar transformações 2D
function apply2DTransform() {
    if (polygons.length === 0 || isCubeActive) return;

    // Pega o último estado antes de começar a transformar
    const originalPolygon = polygons[polygons.length - 1];
    let transformedPolygon = originalPolygon;

    const tx = parseFloat(txInput.value) || 0;
    const ty = parseFloat(tyInput.value) || 0;
    const sx = parseFloat(sxInput.value) || 1;
    const sy = parseFloat(syInput.value) || 1;
    const angle = parseFloat(angleInput.value) || 0;
    const center = getPolygonCenter(originalPolygon);

    // Aplica transformações em ordem: Escala -> Rotação -> Translação
    const T1 = createTranslationMatrix(-center.x, -center.y);
    const S = createScalingMatrix(sx, sy);
    const R = createRotationMatrix(angle);
    const T2 = createTranslationMatrix(center.x + tx, center.y + ty); // Combina translação original e a do input

    let finalMatrix = multiplyMatrices(T2, R);
    finalMatrix = multiplyMatrices(finalMatrix, S);
    finalMatrix = multiplyMatrices(finalMatrix, T1);

    transformedPolygon = transformPolygon(originalPolygon, finalMatrix);

    // Atualiza o polígono na lista e redesenha
    polygons[polygons.length - 1] = transformedPolygon;
    redrawAllPolygons();
}

// Listener para o botão do Cubo 3D
drawCubeBtn.addEventListener('click', () => {
    saveState();
    draw3DCube();
});

// Listeners para os inputs de transformação
txInput.addEventListener('input', () => { isCubeActive ? draw3DCube() : apply2DTransform(); });
tyInput.addEventListener('input', () => { isCubeActive ? draw3DCube() : apply2DTransform(); });
sxInput.addEventListener('input', () => { isCubeActive ? draw3DCube() : apply2DTransform(); });
syInput.addEventListener('input', () => { isCubeActive ? draw3DCube() : apply2DTransform(); });
angleInput.addEventListener('input', () => { isCubeActive ? draw3DCube() : apply2DTransform(); });

// Adiciona um saveState quando o usuário começa a mexer nos inputs
txInput.addEventListener('focus', () => { if (!isCubeActive && polygons.length > 0) saveState(); });
tyInput.addEventListener('focus', () => { if (!isCubeActive && polygons.length > 0) saveState(); });
sxInput.addEventListener('focus', () => { if (!isCubeActive && polygons.length > 0) saveState(); });
syInput.addEventListener('focus', () => { if (!isCubeActive && polygons.length > 0) saveState(); });
angleInput.addEventListener('focus', () => { if (!isCubeActive && polygons.length > 0) saveState(); });
...
function draw3DCube() {
    ...
    arestasDoCubo.forEach(aresta => {
        const linha = bresenhamSync(aresta.p1.x, aresta.p1.y, aresta.p2.x, aresta.p2.y);
