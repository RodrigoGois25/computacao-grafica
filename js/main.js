...
function handleClippingClick(x, y) {
...
document.getElementById('translateBtn').addEventListener('click', () => { if (polygons.length === 0) return; saveState(); const tx = parseFloat(document.getElementById('tx').value) || 0; const ty = parseFloat(document.getElementById('ty').value) || 0; const translationMatrix = createTranslationMatrix(tx, ty); const lastPolygon = polygons.pop(); const newPolygon = transformPolygon(lastPolygon, translationMatrix); polygons.push(newPolygon); redrawAllPolygons(); });
document.getElementById('scaleBtn').addEventListener('click', () => { if (polygons.length === 0) return; saveState(); const sx = parseFloat(document.getElementById('sx').value) || 1; const sy = parseFloat(document.getElementById('sy').value) || 1; const lastPolygon = polygons.pop(); const center = getPolygonCenter(lastPolygon); const T1 = createTranslationMatrix(-center.x, -center.y); const S = createScalingMatrix(sx, sy); const T2 = createTranslationMatrix(center.x, center.y); let finalMatrix = multiplyMatrices(T2, S); finalMatrix = multiplyMatrices(finalMatrix, T1); const newPolygon = transformPolygon(lastPolygon, finalMatrix); polygons.push(newPolygon); redrawAllPolygons(); });
document.getElementById('rotateBtn').addEventListener('click', () => { if (polygons.length === 0) return; saveState(); const angle = parseFloat(document.getElementById('angle').value) || 0; const lastPolygon = polygons.pop(); const center = getPolygonCenter(lastPolygon); const T1 = createTranslationMatrix(-center.x, -center.y); const R = createRotationMatrix(angle); const T2 = createTranslationMatrix(center.x, center.y); let finalMatrix = multiplyMatrices(T2, R); finalMatrix = multiplyMatrices(finalMatrix, T1); const newPolygon = transformPolygon(lastPolygon, finalMatrix); polygons.push(newPolygon); redrawAllPolygons(); });

// --- LÓGICA PARA O CUBO 3D ---
